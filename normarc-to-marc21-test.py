...
def compare(identifier, normarc_path, marc21_path, normarc_source_path, marc21_source_path):
    ...
    with lock:
        ...
        while linenum < len(normarc):
            ...
            if "bibliofil-id" in normarc_line and '*' in normarc_line_comment and normarc_line_comment.split('*')[1].split(' ')[0] in ["100$_", "650$_", "700$_"]:
                normarc_line = re.sub(r'>\d+<', '>X<', normarc_line)
            if "bibliofil-id" in marc21_line and '*' in marc21_line_comment and marc21_line_comment.split('*')[1].split(' ')[0] in ["100$_", "650$_", "700$_"]:
