...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="bibliofil-id">
        ...
        <xsl:variable name="subfield" select="if (count($subfield) = 0) then $context/*:subfield[@code='3' and matches(., '^\d+$')] else $subfield"/>
    ...
    <xsl:template match="*:datafield[@tag='574']">
        <xsl:if test="not(preceding-sibling::*:datafield[@tag='574'])">
            <xsl:for-each select="../*:datafield[@tag='574']">
                <xsl:sort select="*:subfield[@code='a'][1]/replace(text(), '^.*?: *', '')"/>
                <xsl:for-each select="*:subfield[@code='a']">
                    <xsl:variable name="value" select="replace(replace(text(),'^\s*Ori?gi(na|an)l(ens )?tit\w*\s*:?\s*',''), ' *\.$', '')"/>
                    <xsl:variable name="value" select="replace($value,'^\s*Oversatt etter:?\s*', '')"/>
                    <xsl:if test="not($value = 'mangler')">
                        <xsl:call-template name="meta"><xsl:with-param name="property" select="'dc:title.original'"/><xsl:with-param name="value" select="$value"/></xsl:call-template>
                    </xsl:if>
                </xsl:for-each>
    ...
    <xsl:template match="*:datafield[@tag='650']">
        ...
        <xsl:if test="not(preceding-sibling::*:datafield[@tag='650'])">
            ...
            <xsl:for-each select="../*:datafield[@tag='650' and exists(*:subfield[@code='a'])]">
                <xsl:sort select="string-join((*:subfield[@code='a']/upper-case(text()), *:subfield[@code='q']/text(), *:subfield[@code='0']/text()), ' ')"/>
    ...
    <xsl:template match="*:datafield[@tag='697']">
        ...
        <xsl:for-each select="*:subfield[@code='a']">
            <xsl:if test="text() = 'Lydbok med tekst'">
                <xsl:call-template name="meta">
                    <xsl:with-param name="property" select="'dc:subject.keyword'"/>
                    <xsl:with-param name="value" select="text()"/>
